...
from linear_cbm_model import ConceptBottleneckLinear
...
def main():
    ...
    if ds_params:
        if "num_heads" in ds_params and args.num_heads is None:
            args.num_heads = ds_params["num_heads"]
        if "batch_size" in ds_params and args.batch_size is None:
    ...
    if args.checkpoint_path is None:
        ...
        best_model = GAT_LightningModule.load_from_checkpoint(
            best_ckpt_path, model=clf
        )
    ...
    m = best_model.model.to(device)
    train_metrics = evaluate(m, train_loader, device, num_classes)
    val_metrics = evaluate(m, val_loader, device, num_classes)
    test_metrics = evaluate(m, test_loader, device, num_classes)
    ...
    print(
        f"Test:  acc={test_metrics['acc']:.4f}  f1={test_metrics['f1']:.4f}  auc={test_metrics['auc']:.4f}"
    )
